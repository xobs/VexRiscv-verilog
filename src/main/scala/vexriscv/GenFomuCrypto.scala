...
object FomuCryptoSpinalConfig extends spinal.core.SpinalConfig(
  defaultConfigForClockDomains = ClockDomainConfig(
    resetKind = spinal.core.SYNC
  )
)

case class FomuCryptoArgConfig(
  debug : Boolean = false,
  iCacheSize : Int = 4096,
  dCacheSize : Int = 4096,
  mulDiv : Boolean = true,
...
object GenFomuCrypto{
  ...
  def main(args: Array[String]) {
    ...
    val parser = new scopt.OptionParser[FomuCryptoArgConfig]("VexRiscvGen") {
      //  ex :-d    or   --debug
    ...
    val argConfig = parser.parse(args, FomuCryptoArgConfig()).get

    FomuCryptoSpinalConfig.copy(netlistFileName = argConfig.outputFile + ".v").generateVerilog {
      ...
      plugins ++= List(
        ...
        if(argConfig.singleCycleMulDiv) {
          new FullBarrelShifterPlugin(earlyInjection = !argConfig.withMemoryStage)
        }else {
